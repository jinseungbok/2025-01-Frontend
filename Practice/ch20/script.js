...
const $contactSildeCon = document.querySelector("#contact .contact__slide-con");

$contactTabs.forEach((item, idx) => {
  const marginLeft = [0, "-100vw"][idx];

  // 위 코드의 내용을 풀어 쓰면 다음과 같다.
  // let marginLeft;
  // if(idx === 0) {
  //    marginLeft = 0;
  //  } else if(idx === 1) {
  ...
  item.addEventListener("click", () => {
    $contactSildeCon.style.marginLeft = marginLeft;
  });
});
...
// $header.addEventListener('click', () => alert('dd'));
